...
// Borra un movimiento de 'person': por IMPORTE, por CONCEPTO (keyword) o el ÚLTIMO si no hay criterio.
export async function deleteMovement(person, { importe, concepto } = {}) {
  const s = client();
  const r = await s.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `${MOV}!A2:G${MAX_ROW}`, valueRenderOption: "UNFORMATTED_VALUE" });
  const rows = r.data.values || [];
  let target = -1;
  for (let i = 0; i < rows.length; i++) {
    const row = rows[i];
    if (!row || row[0] == null || row[6] !== person) continue; // solo movimientos de esa persona
    if (importe != null) {
      if (typeof row[5] === "number" && Math.abs(row[5] - importe) < 0.01) target = i;
    } else if (concepto) {
      if (String(row[4] || "").toLowerCase().includes(concepto.toLowerCase())) target = i;
    } else {
      target = i; // el último
    }
  }
  if (target < 0) return { notFound: true };
  const rowNum = target + 2;
  const row = rows[target];
  const info = { concepto: row[4] || row[3], importe: row[5], tipo: row[2] };
  await s.spreadsheets.values.batchClear({
    spreadsheetId: SHEET_ID,
    requestBody: { ranges: [`${MOV}!A${rowNum}`, `${MOV}!C${rowNum}:G${rowNum}`] },
  });
  return info;
}

// Cuánto ha GASTADO cada persona este mes (Común + Personal + Fijo personal).
export async function gastosPorPersona(mes, tipos = ["Común", "Personal", "Fijo personal"]) {
  const s = client();
  const r = await s.spreadsheets.values.get({ spreadsheetId: SHEET_ID, range: `${MOV}!A2:G${MAX_ROW}`, valueRenderOption: "UNFORMATTED_VALUE" });
  const rows = r.data.values || [];
  const gasto = tipos;
  ...
  for (const row of rows) {
    const fecha = row[0], tipo = row[2], categoria = row[3], imp = row[5], quien = row[6];
    if (typeof fecha !== "number" || typeof imp !== "number") continue;
    if (categoria === "Ahorro" || categoria === "Préstamo mamá") continue; // traspaso a ahorro / dinero de paso del préstamo de mamá, no cuenta como gasto
    const d = new Date(Date.UTC(1899, 11, 30) + fecha * 86400000); // serial de Sheets -> fecha
    const m = `${d.getUTCFullYear()}-${String(d.getUTCMonth() + 1).padStart(2, "0")}`;
    if (m === mes && gasto.includes(tipo)) tot[quien] = (tot[quien] || 0) + imp;
  }
  return tot; // { Persona: total }
}

// Cambia la CATEGORÍA del último movimiento apuntado por 'person'.
export async function updateLastCategoria(person, nuevaCategoria) {
  const rows = await read(`${MOV}!A2:G${MAX_ROW}`);
  let target = -1;
  for (let i = 0; i < rows.length; i++) {
    const r = rows[i];
  ...
  const r = rows[target];
  return { row: rowNum, concepto: r[4] || r[3], importe: r[5], categoria: nuevaCategoria };
}
