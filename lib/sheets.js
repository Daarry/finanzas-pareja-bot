...
// Borra (vacía) el último movimiento apuntado por 'person'. Deja la fórmula del Mes.
export async function deleteLastMovement(person) {
  const rows = await read(`${MOV}!A2:G${MAX_ROW}`);
  let target = -1;
  for (let i = 0; i < rows.length; i++) {
    const r = rows[i];
  ...
  const rowNum = target + 2;
  const r = rows[target];
  const info = { concepto: r[4] || r[3], importe: r[5], tipo: r[2] };
...
export async function gastosPorPersona(mes) {
  const s = client();
  ...
  const gasto = ["Común", "Personal", "Fijo personal"];
  const tot = {};
