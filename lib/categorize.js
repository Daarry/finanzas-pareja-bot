...
export function transferTipo(text, p1, p2) {
  ...
  const incoming = /\b(me ha pasado|me han pasado|me ha hecho un bizum|me hizo un bizum|me ha ingresado|me han ingresado|me ha transferido|me ha devuelto|me devolvi[oó])\b/.test(t);
  const outgoing = /\bbizum\b|\b(le he pasado|le he dado|le pago|pago a|paga a|transferencia a|le mando|le env[ií]o)\b/.test(t);
  if (incoming) return toPartner ? "Liquidación" : "Ingreso";
  if (outgoing) return toPartner ? "Liquidación" : "Personal";
  return null;
}
